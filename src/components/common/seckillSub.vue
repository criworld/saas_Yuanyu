			...
			<view class="popupBgc">
				...
				<view class="p-4 d-flex j-sb">
					<view>
						...
						<text>（每人限购{{ activityDetailObj.limitBuy }}份）</text>
					</view>
					<view>
						<text class="iconfont jian" :class="{'active_num':car_num <= 1}"
							@tap="num_reduce()">&#xe6d4;</text>
						<text class="px-3 num">{{ car_num }}</text>
						<text class="iconfont jia" :class="{'active_num':car_num >= activityDetailObj.limitBuy}"
							@tap="num_add()">&#xe6d5;</text>
					</view>
				</view>
		...
		<u-overlay :show="isPayCuccess" @click="isPayCuccess = false">
			<view class="warp">
				<view class="rect" @tap.stop>
					<view class="text-gold font-32r my-3">
						{{ hintText }}
					</view>
					...
					<view>
						<view class="text-light-title">
							<view class="mt-4">
								<text class="text-gold font-lg my-2 mr-1">{{ activityDetailObj.name }}</text>
								<text class="text-gold">× {{ car_num }}</text>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			num_add() {
				if (this.car_num >= this.activityDetailObj.limitBuy) {
			...
			commonBtn() {
				...
				seckillActivitySub(params).then(res => {
				...
				this.timer = setInterval(() => {
					seckillSubResult(val).then(res => {
						...
						if (res.step == 'fail') {
							...
							clearInterval(this.timer);
							uni.navigateTo({
								url: '/pages/pay?token=' + this.$store.state.userToken +
									'&orderNo=' + res.orderNo + '&alias=' + this.alias +
									"&pathEntrance=" + 3
							});
						}
					})
