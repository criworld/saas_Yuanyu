<template>
	<view class="con px-3 tab-bar-height" :style="[JSON.parse(themeJson),bgImg]">
		...
		<view class="item">
			<view class="text">已给手机号{{ipone}}发送验证码</view>
		</view>
		<view class="item"  style="margin-top:60rpx">
			...
			<view class="input py-3 d-flex flex-row a-center j-sb">
				<input
					...
					placeholder="请输入验证码"
				/>
				<text @click="sendCode" v-if="!timeout">发送验证码</text>
...
<script>
import { resetPasswordVcode } from '@/common/api.js';
export default {
	data() {
		return {
			centerTitle:"",
	...
	methods: {
		...
		submit() {
			...
			uni.navigateTo({
				url: '/pages/login/pwd?name=' + this.centerTitle + '&vcode=' + this.vcode + '&userName=' + this.userName,
				animationDuration: 0
			});
		},
	}
