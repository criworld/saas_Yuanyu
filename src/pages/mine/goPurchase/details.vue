		...
		<view class="back  px-4 py-4">
			<view class="details d-flex flex-row j-sb a-center">
				...
				<view class="right flex-1">
					...
					>
						<text class="price">
							¥{{ orderDetailData.priceYuan }}
						</text>
						<text class="num">X{{ orderDetailData.num }}</text>
					</view>
				</view>
			</view>
			<view class="list" style="margin-top:30rpx;">
				<view class="item py-3 j-sb d-flex flex-row">
					<text>订单编号</text>
				...
				<view class="item py-3 j-sb d-flex flex-row">
					<text>截止时间</text>
					<text>{{ orderDetailData.payExpireAt }}</text>
				</view>
				<view class="item py-3 j-sb d-flex flex-row" v-if="orderDetailData.status == 2">
					<text>支付时间</text>
					<text>{{ orderDetailData.payAt }}</text>
				</view>
				<view class="item py-3 j-sb d-flex flex-row" v-if="orderDetailData.status == 2">
					<text>支付方式</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'hee_bank_pay'">
						汇付宝云闪付支付
					</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'wallet_pay'">
						余额支付
					</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'hxt_pay'">
						hxt支付
					</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'wx_h5_pay'">
						微信h5支付
					</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'hee_pay'">
						汇付宝支付
					</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'wx_js_pay'">
						微信小程序支付
					</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'ali_h5_pay'">
						支付宝h5支付
					</text>
					<text class="payType" v-if="orderDetailData.payMethod == 'sande_pc_pay'">
						杉德支付
					</text>
				</view>
			</view>
			...
			<view class="" >
				<view class="item-num py-2 d-flex flex-row j-sb a-center">
					<text>金额总计</text>
					<text>¥{{ orderDetailData.realFeeYuan }}</text>
				</view>
				<view class="item-num py-2 d-flex flex-row j-sb a-center">
					<text>实付金额</text>
					<text>¥{{ orderDetailData.realFeeYuan }}</text>
			...
			<view class="d-flex j-sb mx-5" v-if="orderDetailData.status == 1">
				<!-- <u-button
					@click="orderCancel(orderDetailData.orderNo)"
					class="immediatelyBuyCancel"
					type="primary"
					:plain="true"
					text="取消订单"
				></u-button> -->
				<MButton
					class="immediatelyBuyCancel"
					text="取消订单"
					@click.native="orderCancel(orderDetailData.orderNo)"
				></MButton>
				<MButton
...
export default {
	...
	onLoad(option) {
		...
		activityOrderDetail(option.orderNo).then(res => {
	...
	onShow() {
		...
		// 	setTimeout(() => {
		// 		activityOrderDetail(this.getOrderNo).then(res => {
		// 			this.orderDetailData = res;
		// 		});
		// 	}, 1000);
		// } else {
